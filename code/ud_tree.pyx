...
from ud_splitter cimport DFESplitter, UDSplitter
...
cdef class UDDepthFirstTreeBuilder(TreeBuilder):
    """Build a decision tree in depth-first fashion."""

    def __cinit__(self, UDSplitter splitter, SIZE_t min_samples_split,
                  SIZE_t min_samples_leaf, double min_weight_leaf,
                  SIZE_t max_depth, double min_impurity_decrease,
                  double min_impurity_split):
        self.splitter = splitter
    ...
    cpdef build(self, Tree tree, object X, np.ndarray y,
                np.ndarray sample_weight=None, np.ndarray feature_bias=None):
        """Build a decision tree from the training set (X, y)."""
        ...
        # Parameters
        cdef UDSplitter splitter = self.splitter
        cdef SIZE_t max_depth = self.max_depth
        cdef SIZE_t min_samples_leaf = self.min_samples_leaf
        cdef double min_weight_leaf = self.min_weight_leaf
        cdef SIZE_t min_samples_split = self.min_samples_split
        ...
        with nogil:
            ...
            while not stack.is_empty():
                ...
                if not is_leaf:
                    splitter.ud_node_split(impurity, &split, &n_constant_features, feature_bias_ptr)
